...
/** Work observed when a push begins; later marks must survive its retirement. */
export interface PushSnapshot {
  readonly state: SidecarState;
  readonly dirtyPaths: readonly string[];
  readonly generation: number;
  readonly bulkGeneration: number;
}
...
export class Sidecar {
  ...
  private async refresh(): Promise<SidecarState> {
    await this.load();
  ...
  read(): Promise<SidecarState> {
    const next = this.chain.then(() => this.refresh());
    this.chain = next.catch(() => undefined);
    return next;
  }

  beginPush(): Promise<PushSnapshot> {
    const next = this.chain.then(async () => {
      const state = await this.refresh();
      return {
        state,
        dirtyPaths: [...this.dirty],
        generation: this.generation,
        bulkGeneration: this.bulkGeneration,
      };
    });
    this.chain = next.catch(() => undefined);
    return next;
  ...
  /**
   * Atomically retire only the observed work. The old journal stays intact
   * until its replacement contains all foreign and newly marked paths.
   * Generations distinguish a second mark of an already dirty ancestor. Its
   * existing journal entry remains durable throughout the push, so a crash
   * before replacement safely replays it without needing persisted counters.
   * Like scalar updates, cross-process access to one cache requires core's lock.
   */
  retirePush(
    ...
    const next = this.chain.then(async () => {
      await this.refresh();
      const retained = new Set(this.dirty);
      for (const path of observed.dirtyPaths) {
        if (
          completed(path) &&
          (this.dirtyGenerations.get(path) ?? 0) <= observed.generation
        ) retained.delete(path);
      }
      for (const path of retainedRoots) {
        retained.add(path);
        this.dirtyGenerations.set(path, ++this.generation);
      }
      // Do not clear/reappend: interruption must never erase another scope.
      ...
      for (const path of this.dirtyGenerations.keys()) {
        if (!retained.has(path)) this.dirtyGenerations.delete(path);
      }
      if (complete) {
        if (
          observed.state.bulkInvalidated &&
          observed.bulkGeneration === this.bulkGeneration
        ) this.scalars!.bulkInvalidated = false;
        this.scalars!.pushBootstrapped = true;
        // Write after the journal: a crash between them preserves the old
        // bulk obligation and merely repeats work, never loses pending work.
        await writeScalars(this.cachePath, this.scalars!);
      }
      return this.snapshot();
    });
  ...
  recordDirty(relPath: string | undefined): Promise<SidecarState> {
    ...
    const next = this.chain.then(async () => {
      ...
      const ancestor = this.dirty.has(relPath)
        ? relPath
        : this.dirtyAncestor(relPath);
      this.dirtyGenerations.set(ancestor ?? relPath, ++this.generation);
      if (ancestor !== null) return this.snapshot();
      this.dirty.add(relPath);
      if (this.dirty.size > MAX_DIRTY_PATHS) {
        // Degrade to a full walk rather than tracking an unbounded set.
        this.bulkGeneration++;
        this.scalars!.bulkInvalidated = true;
        // Persist the full-walk obligation before discarding individual marks.
        await writeScalars(this.cachePath, this.scalars!);
        this.dirty.clear();
        this.dirtyGenerations.clear();
        await this.persistJournal();
        return this.snapshot();
      }
  ...
  private dirtyAncestor(relPath: string): string | null {
    let idx = relPath.lastIndexOf("/");
    while (idx > 0) {
      if (this.dirty.has(relPath.slice(0, idx))) return relPath.slice(0, idx);
