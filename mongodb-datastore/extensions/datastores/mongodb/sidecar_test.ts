...
Deno.test("isSecretsPath matches the vault tier only", () => {
  ...
  // Could be the vault tier of namespace "outputs"; do not upload it.
  assertEquals(isSecretsPath("outputs/secrets/foo"), true);
});
  ...
  assertEquals(filter, {
    $or: [
      // Known prefix: incremental, so old tombstones stay out of the cursor.
      {
        ...
        updatedAt: { $gte: new Date("2026-09-04T00:00:00.000Z") },
      },
